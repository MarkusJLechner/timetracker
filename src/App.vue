<template>
  <div class="h-screen bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-5 flex flex-col items-center justify-center">
    <!-- Main Container -->
    <div
      :class="[
    ...
    >
      <div class="relative rounded-2xl shadow-xl p-6 max-w-md h-full w-full  flex flex-col items-center bg-gradient-to-tl from-[#C44ECB] to-[#895DF4] ">
        ...
        <div class="text-center text-white text-4xl font-bold mb-6">
          {{ currentTimer }}
          <span v-if="isRunning" class="text-sm block mt-2">Current: <span class="text-yellow-300">{{ timerEvents[timerEvents.length - 1].details }}</span></span>
        </div>
        <!-- Task Input -->
    ...
    <div class="flex flex-wrap justify-center w-full max-w-4xl mt-4 gap-4">
      <!-- Events List -->
      <div class="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-2xl shadow-xl p-4 w-full max-w-sm">
        <div class="flex justify-between items-center mb-2">
          <div class="text-white text-xl font-semibold">Events</div>
          <button @click="clearEvents" class="text-white underline focus:outline-none text-sm">Clear</button>
        </div>
        <ul class="text-white overflow-y-auto max-h-40 space-y-2">
          <li v-for="(event, index) in reversedTimerEvents" :key="index"
            ...
            <div class="flex justify-between items-center mt-1">
              ...
              <div class="text-yellow-300">{{ formatDuration(event.startTime, event.endTime) }}</div>
            </div>
          </li>
        </ul>
      </div>
...
function startTimer() {
  ...
  intervalId = setInterval(() => {
    ...
    const elapsedTime = Math.floor((currentTime - timerEvents.value[timerEvents.value.length - 1].startTime) / 1000)
